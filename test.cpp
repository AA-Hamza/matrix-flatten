...
template <typename T>
using stlVector3D = std::vector<std::vector<std::vector<T>>>;


template <typename T>
void populateMatrix(stlVector3D<T> &matrix3D) {
...
template <typename T>
int assertEquals(stlVector3D<T> &matrix3D, flattenedVector<T> &flattened) {
    size_t n = matrix3D.size();
    size_t m = matrix3D[0].size();
    size_t p = matrix3D[0][0].size();
        ...
        for (size_t j = 0; j < m; ++j) {
            for (size_t k = 0; k < p; ++k) {
                if (matrix3D[i][j][k] != flattened[flattendIndex] || matrix3D[i][j][k] != flattened.at(i, j, k)) {
                    std::cout << "Assertion Failed at flattend[" << flattendIndex << "] = " << flattened[flattendIndex] << std::endl;
